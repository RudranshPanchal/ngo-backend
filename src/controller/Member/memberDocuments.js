...
const generateIdCardBuffer = async (member) => {
  ...
  try {
    const baseUrl = process.env.BACKEND_URL || "https://res.cloudinary.com/";
...
export const issueIdCard = async (req, res) => {
  try {
    ...
    const pdfBuffer = await generateIdCardBuffer(member);
    if (pdfBuffer.length === 0)
      throw new Error("Generated PDF buffer is empty");

    let cloudinaryUrl = "";
    try {
      cloudinaryUrl = await uploadBufferWithPublicId(
        pdfBuffer,
    ...
    const updatedMember = await Member.findByIdAndUpdate(
      member._id,
      {
        $set: {
          idCardIssued: true,
          idCardCloudinaryUrl: cloudinaryUrl,
          idCardPDF: pdfBuffer,
...
export const issueAppointmentLetter = async (req, res) => {
  try {
    ...
    const RIGHT_BOUNDARY = pageWidth - PAGE_MARGIN;
    ...
    doc
      ...
      .rect(
        borderMargin + 6,
        borderMargin + 6,
        pageWidth - (borderMargin * 2 + 12),
        pageHeight - (borderMargin * 2 + 12),
      )
      .stroke();

    if (fs.existsSync(logoPath)) {
      doc.image(logoPath, LEFT_BOUNDARY, 45, { width: 50 });
    }
    doc
      .font("Playfair-Bold")
      .fontSize(24)
      .fillColor(PRIMARY)
      .text("ORBOSIS FOUNDATION", LEFT_BOUNDARY + 60, 45);
    doc
    ...
    doc.text("123 NGO Street, Social City", contactX, 45, {
      align: "right",
      width: RIGHT_COL_WIDTH,
    });
    doc.text("India - 452001", contactX, 58, {
      align: "right",
      width: RIGHT_COL_WIDTH,
    });
    doc.text("contact@orbosis.org", contactX, 71, {
      align: "right",
      width: RIGHT_COL_WIDTH,
    });
    doc.text("+91 98765 43210", contactX, 84, {
      align: "right",
      width: RIGHT_COL_WIDTH,
    });
    doc
      .moveTo(LEFT_BOUNDARY, 105)
      .lineTo(RIGHT_BOUNDARY, 105)
      .lineWidth(1)
      .strokeColor(GOLD)
      .stroke();
    ...
    doc
      ...
      .fontSize(10)
      .fillColor(TEXT_MAIN)
      .text(`Ref No: OF/MEM/${member.memberId}`, LEFT_BOUNDARY, currentY);
    doc.text(`Date: ${today}`, RIGHT_BOUNDARY - RIGHT_COL_WIDTH, currentY, {
      width: RIGHT_COL_WIDTH,
      align: "right",
    });
    ...
    doc
      ...
      .fontSize(14)
      .fillColor(PRIMARY)
      .text(recipientName, LEFT_BOUNDARY, currentY);
    currentY += 22;
    doc
      .font("OpenSans-Regular")
      .fontSize(10)
    ...
    doc
      ...
      .fontSize(12)
      .fillColor(TEXT_MAIN)
      .text("SUBJECT: APPOINTMENT LETTER", 0, currentY, {
        align: "center",
        underline: true,
      });
    ...
    const startDateStr = new Date(
      startDate || member.approvedAt || new Date(),
    ...
    doc
      .font("OpenSans-Regular")
      .fontSize(11)
      .text(`Dear ${recipientName},`, LEFT_BOUNDARY, currentY);
    currentY += 30;
    doc.text(
      `We are pleased to inform you that you have been appointed as a ${role || "Member"} of Orbosis Foundation, effective from ${startDateStr}. We were very impressed with your background and believe that your skills and experience will be a valuable asset to our organization.`,
      LEFT_BOUNDARY,
      currentY,
      bodyOptions,
    );
    ...
    doc
      .font("OpenSans-Bold")
      .fontSize(10)
      .text("Accepted By:", LEFT_BOUNDARY, sigY);
    doc
      .font("DancingScript")
      .fontSize(18)
      .text(recipientName, LEFT_BOUNDARY, sigY + 25);
    doc
      .font("OpenSans-Regular")
      .fontSize(10)
      .text(`(${recipientName})`, LEFT_BOUNDARY, sigY + 50);

    const rightSigX = RIGHT_BOUNDARY - RIGHT_COL_WIDTH;
    doc
      .font("OpenSans-Bold")
      .fontSize(10)
      .text("For Orbosis Foundation", rightSigX, sigY, {
        width: RIGHT_COL_WIDTH,
        align: "right",
      });
    doc
      .font("DancingScript")
      .fontSize(20)
      .text("Authorized Signatory", rightSigX, sigY + 25, {
        width: RIGHT_COL_WIDTH,
        align: "right",
      });
    doc
      .font("OpenSans-Regular")
      .fontSize(10)
      .text("Authorized Signatory", rightSigX, sigY + 50, {
        width: RIGHT_COL_WIDTH,
        align: "right",
      });
    ...
    doc
      .lineWidth(1)
      .strokeColor(GOLD)
      .moveTo(LEFT_BOUNDARY, footerY)
      .lineTo(RIGHT_BOUNDARY, footerY)
      .stroke();
    doc
      .font("OpenSans-Regular")
      .fontSize(9)
      .fillColor(TEXT_LIGHT)
      .text(
        "www.orbosisfoundation.org | Reg. No: 12345/2023",
        0,
        footerY + 10,
        { align: "center" },
      );
    ...
    let cloudinaryUrl = await uploadBufferWithPublicId(
      pdfBuffer,
...
export const downloadAppointmentLetter = async (req, res) => {
  try {
    ...
    if (member.appointmentLetterPDF) {
      res.setHeader("Content-Type", "application/pdf");
      const bufferData =
        member.appointmentLetterPDF.buffer || member.appointmentLetterPDF;
