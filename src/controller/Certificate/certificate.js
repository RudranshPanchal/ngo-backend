...
function drawDonationCertificate(doc, cert) {
  ...
  const formattedName =
    ...
    recipientName.slice(1).toLowerCase();
  ...
  doc.fontSize(10).text("Authorized Signatory", 108, sigY + 24);
...
function drawVolunteerTemplate(doc, { recipientName, issueDate }) {
  ...
  doc.fontSize(30).text("Volunteer Certificate", { align: "center" });
  ...
  doc.fontSize(14).fillColor("#444").text("Presented to", { align: "center" });
  ...
  // ---- DATE ----
  doc.fontSize(12).fillColor("#444").text(issueDate, 100, footerY);
  ...
  doc.fontSize(10).text("Date", 120, footerY + 18);

  // ---- SIGNATURE ----
  const signX = pageWidth - 240;

  doc.fontSize(12).text("Pooja Mogal", signX, footerY);
  ...
  doc.fontSize(10).text("Authorized Signatory", signX + 15, footerY + 18);
}

//      1. CREATE CERTIFICATE (NO PDF HERE!)
export const generateCertificate = async (req, res) => {
  try {
    const { recipientName, issueDate, description, email, role, userId } =
      req.body;
    ...
    // Create notification for member if userId provided
...
export const downloadCertificatePDF = async (req, res) => {
  try {
    ...
    doc.on("end", () => {
      ...
      res.setHeader(
        "Content-Disposition",
        `attachment; filename=${cert.recipientName}.pdf`,
      );
      res.send(pdf);
    });
...
export const sendCertificateEmailController = async (req, res) => {
  try {
    ...
    if (!email || !role) {
      return res
        .status(400)
        .json({ success: false, message: "Email + role required" });
    }
